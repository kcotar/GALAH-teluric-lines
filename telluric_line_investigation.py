...
spectra_file_csv = 'galah_dr51_ccd3_6475_6745_interpolated_wvlstep_0.06_spline_barycentric.csv'
...
spectral_data = pd.read_csv(galah_data_dir + spectra_file_csv, sep=',', header=None, na_values='nan', usecols=idx_read[0]).values
spectal_data_size = np.shape(spectral_data)
print spectal_data_size
wvl_read = wvl_values[idx_read]

out_dir = 'Ca_lines-red_fit_complete'
if os.path.exists(out_dir) == False:
    os.mkdir(out_dir)
os.chdir(out_dir)

prefix = 'tellurics_red'
...
if not os.path.isfile(tellurics_wvl_txt):
    ...
    for field_id in selected_observation_fields:  # filter by date
        ...
        for row in spectra_row[0]:
            ...
            axes[0].plot(wvl_read, row_data, color='blue', alpha=0.05, linewidth=0.8)
