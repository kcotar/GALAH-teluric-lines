...
os.chdir('Sky_spectrum_dr52')
for field_id in [140305002601]: #np.unique(observation_fields)[500:]:
    print field_id
    idx_read = spectra_row = np.where(field_id == observation_fields)
    for i_row in idx_read[0]:
        obj_data = galah_param[i_row]
        # dr51
        spectra_data = fits.open(spectra_data_dir51+str(field_id)[:6]+'/combined/'+str(obj_data['sobject_id'])+'3.fits')
        # dr52
        # spectra_data = fits.open(spectra_data_dir52+str(field_id)[:6]+'/standard/com/'+ str(obj_data['sobject_id'])+'3.fits')
        #
    ...
    plt.xlim((6490, 6520))
    plt.ylim((-200, 700))
    plt.savefig(str(field_id)+'_51.png', dpi=200)
    plt.close()
